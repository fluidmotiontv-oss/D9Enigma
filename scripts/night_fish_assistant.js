...
function injectWidgetElements() {
    ...
    holoOverlay.addEventListener('click', (e) => {
        // Close if click target is the close button or clicking outside the container
        if (e.target.classList.contains('holo-close-btn') || e.target === holoOverlay) {
            holoOverlay.style.display = 'none';
            config.holoActive = false;
            return;
        }

        // Summon random asset on click inside the holographic overlay
        const assetName = AssetLibrary.getRandomAssetKey();
        if (assetName) {
            const x = e.clientX;
            const y = e.clientY;
            const behaviors = ['drift', 'orbit'];
...
function cycleAssistantSpeech() {
    const phrase = COGNITION_DATA[messageIndex][config.activeLang] || COGNITION_DATA[messageIndex]['EN'];
    speak(phrase);
...
function init() {
    ...
    // Play the swooping, bubbling welcome animation
