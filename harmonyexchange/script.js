...
const auctionArtworks = [
    ...
    { title: "Matauranga Time-Warp", artist: "Tim Dwen", initialBid: 35, avatar: "🌀", desc: "Off-grid temporal helix canvas" },
...
function appendRapLine(speaker, text, suffix = '') {
    ...
    else if (speaker === 'TIM DWEN (YOU)') { color = '#ffffff'; avatar = '🚀'; }
...
window.placeLiveBid = function() {
    ...
    liveBidder = "TIM DWEN (YOU)";
    
    document.getElementById('liveArtBid').textContent = `${liveBid} HN`;
    document.getElementById('liveArtBidder').textContent = "TIM DWEN (YOU)";
    
    appendRapLine("TIM DWEN (YOU)", "This piece is legendary, I'm locking down the zone!", `(Bids ${liveBid} HN!)`);
};
...
window.spitUserDiss = function() {
    ...
    appendRapLine("TIM DWEN (YOU)", lyric, "");
};
...
function endAuction() {
    ...
    if (liveBidder === "TIM DWEN (YOU)") {
        if (deductVaultBalance(liveBid)) {
            ...
            if (feed) feed.innerHTML += `<div style="color: var(--highlight); font-weight: bold; margin-top: 10px;">🏆 [SYSTEM] SOLD! Tim Dwen wins "${art.title}" for ${liveBid} HN! Art piece synced to Inventory.</div>`;
...
function appendRadioLine(speaker, text, avatar = '📻', color = 'var(--accent)') {
    ...
    // If speech synthesis is enabled, we read it
...
window.sendRadioTalkback = function() {
    ...
    inputEl.value = '';
    appendRadioLine("TIM DWEN (YOU)", text, "📢", "#ffffff");
    
    // 40% chance a caller responds after 2.5 seconds
    setTimeout(() => {
        if (Math.random() < 0.40) {
...
const mockCallerResponses = [
    { name: "Gino", text: "That is clean! Upcycling is pure Rongoā for the soil.", avatar: "🍄", color: "var(--highlight)" },
    { name: "Misaki", text: "I can visualize that aesthetic! Standard models don't match the organic design.", avatar: "🎨", color: "var(--alert)" },
    { name: "Conan", text: "Verified on-grid. Let's record the audio using Ardour and spin a new loop.", avatar: "📡", color: "#ffcc00" },
    { name: "Corey", text: "Love that talkback flow! Turn it up, the sub bass in my truck is ready.", avatar: "🎧", color: "var(--accent)" },
    { name: "Night Fish", text: "Sovereign signal is strong here. Beautiful vibes.", avatar: "🐠", color: "var(--highlight)" }
];

function triggerMockCallerResponse() {
    const resp = mockCallerResponses[Math.floor(Math.random() * mockCallerResponses.length)];
    appendRadioLine(resp.name, resp.text, resp.avatar, resp.color);
}
...
const mockCallersList = [
    {
        ...
        color: "var(--highlight)",
        lines: [
            "Just planted some wild Kawakawa in reclaimed planters. Keep the soil rich!",
            "Anyone need four pallets of cedar timber? Free at the block 3 depot.",
            "Upcycled compost bins from plastic drums is the way to go.",
            "The Beach Clean Up needs a crew at dawn. Bring your flax sacks!"
        ]
    },
    {
        name: "Misaki",
        ...
        color: "var(--alert)",
        lines: [
            "Weaving some Harakeke nets to scoop up microplastics. Functional art!",
            "I'm putting solar lamps into vintage milk bottles. Looks stunning in dark mode.",
            "Upcycling makes trash look beautiful. Digital-organic fusion.",
            "Can we host a design showcase for the best word sculptures?"
        ]
    },
    {
        name: "Conan",
        ...
        color: "#ffcc00",
        lines: [
            "Salvaged two 12V lead-acid batteries. Ready for the solar transmitter shed.",
            "Antenna is aligned. Broadcast signal covers 15 kilometers on the FM band.",
            "Remember to back up your remix files before opening Ardour!",
            "Sovereign packet network is active. No central server can shut this down."
        ]
    },
    {
        name: "Corey",
        ...
        color: "var(--accent)",
        lines: [
            "Got the e-waste synthesizer humming. The filter sweep is dusty!",
            "Donated a stack of old audio cables to the clean up. Use them for bindings.",
            "Let's remix the ocean wave soundscape on Kdenlive.",
            "Spoken word fits perfectly on this ambient synth pad."
        ]
    },
    {
        name: "Night Fish",
        ...
        color: "var(--highlight)",
        lines: [
            "Clean water, clean minds, clean air. Community power is the focus.",
            "Just dropped off some free tools at the upcycle depot. Help yourself!",
            "Breathe in, breathe out. Healing notes on the wind.",
            "Discerning the true frequency of this valley through upcycled ears."
        ]
    }
];
...
window.donateJunk = function() {
    ...
    const note = {
        id: 'HN-' + Math.floor(100000 + Math.random() * 900000),
        value: reward,
        creator: 'Community Upcycle Depot',
        keyphrase: 'junk-donation-' + junkItem.toLowerCase().replace(/[^a-z0-9]/g, '-'),
        mintDate: new Date().toISOString(),
        rootVal: currentRoot,
        d9Time: currentD9Time
    };
...
window.pledgeCleanup = function() {
    const cost = 5;
    if (getVaultTotal() < cost) {
        log(`Pledge failed: Insufficient HN in vault (5 HN required).`, "WARN");
        appendRadioLine("RADIO", "Your vault balance is too low to fund a cleanup pledge. Go donate some junk!", "📻", "var(--alert)");
        return;
    }
    
    if (deductVaultBalance(cost)) {
        log(`Pledged 5 HN to the Matariki Beach Clean Up Fund!`, "SUCCESS");
        appendRadioLine("TIM DWEN (YOU)", `Pledging 5 H-Notes to help cleanup and restore the local ecosystem. Keep it pure!`, "🧹", "#ffffff");
        
        setTimeout(() => {
            appendRadioLine("HOST TIM", `Mīharo! (Amazing!) Tim Dwen's pledge received. 5 H-Notes routed to the restoration wallet.`, "📻", "var(--highlight)");
        }, 1500);
    }
};
...
window.exportRadioTranscript = function() {
    ...
    if (lines.length === 0) {
        lines.push(feed.innerText || feed.textContent);
    }
...
function setupRadioShow() {
    updateRadioSegmentUI();
    // Rotate every 30 seconds
    setInterval(rotateRadioSegment, 30000);
    // Add a random caller statement every 18 seconds
