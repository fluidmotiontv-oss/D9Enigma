...
// --- WEBSOCKET CLIENT & SYNC PROTOCOL ---
let ws = null;
...
function connectWebSocket() {
    ...
    ws.onopen = () => {
        log("Connected to Dragon 9 Fabric Server (WS://localhost:9100)", "success");
        if (reconnectTimeout) {
    ...
    ws.onclose = () => {
        if (!reconnectTimeout) {
            log("Disconnected from Fabric Server. Operating in standalone simulation mode.", "warn");
        }
        ws = null;
    ...
    ws.onerror = (err) => {
        // Silent error to prevent console spam when server is down
    };
    ...
    ws.onmessage = (event) => {
        try {
            ...
            console.error("Error decoding message payload:", e);
        }
    };
}
...
function handleSocketMessage(data) {
    switch (data.action) {
        case "SYNC_STATE":
            if (data.node_states) {
                Object.keys(data.node_states).forEach(idxStr => {
                    const idx = parseInt(idxStr, 10);
                    if (state.nodes[idx]) {
                        state.nodes[idx].status = data.node_states[idxStr];
                    }
                });
                updateNodeListDOM();
                updateGlowTheme();
            ...
            break;
        case "GOSSIP_PULSE":
            triggerLocalGossipPulseAtNode(data.origin);
            break;
        case "CALIBRATE":
            runLocalCalibrationSweep();
            break;
        case "TEST_FLOOD":
            runLocalFloodNetworkPackets();
            break;
        case "HEAL":
            runLocalHealAllFaults();
            break;
        case "NODE_STATUS_CHANGE":
            ...
            if (state.nodes[idx]) {
                ...
                if (status === 'offline') {
                    node.drift = 0;
                    log(`CRITICAL: Connection lost with Node '${node.name}' (IP: ${node.ip}).`, "error");
                    playFaultTone();
                } else {
                    node.drift = (Math.random() - 0.5) * 0.012;
            ...
            break;
        case "NODE_JOINED":
            log(`External raw TCP node registered: '${data.node_id}' at ${data.ip} (${data.type})`, "success");
            playBeep(750, 'sine', 0.1, 0.04);
            break;
    }
}
...
window.toggleSelectedNodeState = function() {
    ...
    if (ws && ws.readyState === WebSocket.OPEN) {
        ...
        runLocalToggleNodeState(state.selectedNodeIndex, newStatus);
    }
};

function runLocalToggleNodeState(index, status) {
    const node = state.nodes[index];
    node.status = status;
    if (status === 'online') {
        node.drift = (Math.random() - 0.5) * 0.012;
        log(`Node '${node.name}' booted up (Local Simulation). IP: ${node.ip}`, "success");
        playBeep(660, 'sine', 0.15, 0.05);
    } else {
        node.drift = 0;
        log(`CRITICAL: Connection lost with Node '${node.name}' (Local Simulation).`, "error");
        playFaultTone();
    }
    selectNode(index);
    updateNodeListDOM();
...
window.triggerSingleNodeGossip = function() {
    ...
    if (ws && ws.readyState === WebSocket.OPEN) {
        ws.send(JSON.stringify({
            action: "GOSSIP_PULSE",
            origin: state.selectedNodeIndex
        }));
    } else {
        triggerLocalGossipPulseAtNode(state.selectedNodeIndex);
    }
};
...
function getMasterSyncTime() {
    ...
    return getDragonSync(new Date(Date.now() + drift - (new Date().getTimezoneOffset() * 60000)));
}
...
window.triggerGossipPulse = function() {
    ...
    if (ws && ws.readyState === WebSocket.OPEN) {
        ...
        triggerLocalGossipPulseAtNode(originIdx);
    }
};

function triggerLocalGossipPulseAtNode(originIdx) {
...
function runLocalCalibrationSweep() {
    if (state.calibrating) return;
...
window.floodNetworkPackets = function() {
    if (ws && ws.readyState === WebSocket.OPEN) {
        ws.send(JSON.stringify({ action: "TEST_FLOOD" }));
    } else {
        runLocalFloodNetworkPackets();
    }
};

function runLocalFloodNetworkPackets() {
    ...
    for (let i = 0; i < 6; i++) {
        setTimeout(() => {
            ...
            if (onlineNodes.length > 0) {
                ...
                triggerLocalGossipPulseAtNode(randOrigin);
            }
        }, i * 400);
    }
}
...
function runLocalHealAllFaults() {
...
function init() {
    ...
    // 5. Connect to the real-time syncing WebSocket server
    connectWebSocket();
